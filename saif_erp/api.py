...
def dashboard_data():
	...
	m = frappe.db.sql(
		...
		f"from {jo} where docstatus=1", as_dict=True,
	)[0]
	...
	by_accountant = frappe.db.sql(
		f"""select jo.accountant user, count(*) value from {jo} jo
		where jo.docstatus = 1 group by jo.accountant order by value desc limit 6""", as_dict=True,
	)
	for r in by_accountant:
		r["label"] = (frappe.utils.get_fullname(r["user"]) or r["user"] or "—").split("@")[0]
